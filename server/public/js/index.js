...
/***/ (function(module, exports, __webpack_require__) {
	...
	function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } } //加载模块
		...
		_createClass(Lottery, [{
			...
			value: function updateState() {
				...
				this.getState().then(function (res) {
					...
					(0, _jquery2.default)(self.issue_el).text(res.issue); //更新当前的期号
					self.countdown(res.end_time, function (time) {
						//更新倒计时
						(0, _jquery2.default)(self.countdown_el).html(time); //更新时间
					}, function () {
						setTimeout(function () {
			...
			value: function initEvent() {
				var self = this;
				//添加base.js中的dom事件
				(0, _jquery2.default)('#plays').on('click', 'li', self.changePlayNav.bind(self)); //切换玩法
				(0, _jquery2.default)('.boll-list').on('click', '.btn-boll', self.toggleCodeActive.bind(self)); //号码选中
				(0, _jquery2.default)('#confirm_sel_code').on('click', self.addCode.bind(self)); //添加号码
				(0, _jquery2.default)('.dxjo').on('click', 'li', self.assistHandle.bind(self)); //操作区 大小 奇偶 清除
				(0, _jquery2.default)('.qkmethod').on('click', '.btn-middle', self.getRandomCode.bind(self)); //随机号码
			}
		}]);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //基础设置
	/**
	 * 彩种的基本信息  玩法 奖金 基本数据
	 */
	...
	var Base = function () {
		...
		_createClass(Base, [{
			...
			value: function toggleCodeActive(e) {
				var self = this;
				var $cur = (0, _jquery2.default)(e.currentTarget); //获取当前选中的dom   事件代理
			...
			value: function changePlayNav(e) {
				var self = this;
				var $cur = (0, _jquery2.default)(e.currentTarget); //currentTarget  发生在Dom上的事件 子元素上
				$cur.addClass('active').siblings().removeClass('active');
				self.cur_play = $cur.attr('desc').toLocaleLowerCase(); //将R1~R8 转换为小写
				(0, _jquery2.default)('#zx_sm span').html(self.play_list.get(self.cur_play).tip); //切换过程
			...
			value: function getRandom(num) {
				var arr = [],
				    index = void 0;
				//定义随机空间
				var number = Array.from(this.number); //类似数组集合转成数组
				while (num--) {
			...
			value: function getRandomCode(e) {
				e.preventDefault();
				var num = e.currentTarget.getAttribute('count'); //随机生成的数量
	   ...
	   _createClass(Timer, [{
	      ...
	      value: function countdown(end, update, handle) {
	         ...
	         if (now - end > 0) {
	            ...
	            //全部转换成毫秒数
	            var last_time = end - now; //以毫秒形式剩余时间
	            var px_d = 1000 * 60 * 60 * 24; //识别一天多少毫秒
	            var px_h = 1000 * 60 * 60; //一小时时间
	            var px_m = 1000 * 60; //分
	            var px_s = 1000; //秒
	...
	var Calculate = function () {
	  ...
	  _createClass(Calculate, [{
	    ...
	    value: function computeCount(active, play_name) {
	      ...
	      var arr = new Array(active).fill('0'); //传递当前号码的个数 生成一个active的数组
	    ...
	    value: function combine(arr, size) {
	      ...
	      (function f(arr, size, result) {
	        var arrLen = arr.length; //判断当前数组的长度
	        if (size > arrLen) {
	          return;
	        }
	        if (size === arrLen) {
